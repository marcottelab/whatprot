...
#include "cyclic-detach-transition.h"
...
CyclicDetachTransition::CyclicDetachTransition(double p_detach)
        : p_detach(p_detach) {}

void CyclicDetachTransition::prune_forward(KDRange* range,
                                           bool* allow_detached) {
    pruned_range = *range;
...
void CyclicDetachTransition::prune_backward(KDRange* range,
                                            bool* allow_detached) {
...
PeptideStateVector* CyclicDetachTransition::forward(
...
double CyclicDetachTransition::forward(const Tensor& input,
                                       Tensor* output) const {
...
PeptideStateVector* CyclicDetachTransition::backward(
        const PeptideStateVector& input, unsigned int* num_edmans) const {
    PeptideStateVector* output = new PeptideStateVector(pruned_range);
...
void CyclicDetachTransition::backward(const Tensor& input,
                                      double p_detached,
                                      Tensor* output) const {
    ConstTensorIterator* in_itr = input.const_iterator(pruned_range);
    TensorIterator* out_itr = output->iterator(pruned_range);
...
void CyclicDetachTransition::improve_fit(
...
void CyclicDetachTransition::improve_fit(const Tensor& forward_tsr,
    ...
    delete f_itr;
    delete b_itr;
    fitter->p_cyclic_detach_fit.numerator +=
    ...
    fitter->p_cyclic_detach_fit.denominator +=
